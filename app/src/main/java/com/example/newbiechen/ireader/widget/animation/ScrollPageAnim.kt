 ...
 * 3. 弱网环境下，显示的问题
 */
class ScrollPageAnim(w: Int, h: Int, marginWidth: Int, marginHeight: Int, view: View,
                     listener: OnPageChangeListener) : PageAnimation(w, h, marginWidth,
        marginHeight, view, listener) {
    companion object {
        private const val VELOCITY_DURATION = 1000 // 滑动追踪的时间
    }

    private var mVelocity: VelocityTracker? = null
    // 整个Bitmap的背景显示
    private val mBgBitmap: Bitmap = Bitmap.createBitmap(mScreenWidth, mScreenHeight, Bitmap.Config.RGB_565)
    // 下一个展示的图片
    private var mNextBitmap: Bitmap? = null
    // 被废弃的图片列表
    private val mScrapViews: ArrayDeque<BitmapView> = ArrayDeque(2)
    // 正在被利用的图片列表
    ...
    init {
        for (i in 0..1) {
            ...
            bitmapView.bitmap = Bitmap.createBitmap(mViewWidth, mViewHeight, Bitmap.Config.RGB_565)
            bitmapView.srcRect = Rect(0, 0, mViewWidth, mViewHeight)
            bitmapView.destRect = Rect(0, 0, mViewWidth, mViewHeight)
            bitmapView.top = 0
            bitmapView.bottom = bitmapView.bitmap.height
    ...
    private lateinit var downIt: MutableIterator<BitmapView>
    /**
     * 创建View填充底部空白部分
     *
    ...
    private lateinit var upIt: MutableIterator<BitmapView>
    /**
     * 创建View填充顶部空白部分
     *
    ...
    private lateinit var tmpView: BitmapView
...
/*        //设置背景透明
    ...
    override fun startAnim() {
        isRunning = true
        mScroller.fling(0, mTouchY.toInt(), 0, mVelocity!!.yVelocity.toInt(),
