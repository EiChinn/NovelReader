...
object BrightnessUtils {
    ...
    fun isAutoBrightness(activity: Activity): Boolean {
        var isAuto = false
        try {
            isAuto = Settings.System.getInt(activity.contentResolver, Settings.System.SCREEN_BRIGHTNESS_MODE) == Settings.System.SCREEN_BRIGHTNESS_MODE_AUTOMATIC
    ...
    fun getScreenBrightness(activity: Activity): Int {
        return if (isAutoBrightness(activity)) getAutoScreenBrightness(activity) else getManualScreenBrightness(activity)
    }
