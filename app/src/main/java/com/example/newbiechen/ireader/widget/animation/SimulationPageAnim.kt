...
class SimulationPageAnim(w: Int, h: Int, view: View, listener: OnPageChangeListener) : HorizonPageAnim(w, h, view, listener) {
    ...
    private fun drawCurrentPageShadow(canvas: Canvas) {
        ...
        if (mIsRTandLB) {
            ...
            degree = Math.PI / 4 - Math.atan2((mTouchY - mBezierControl1.y).toDouble(), (mTouchX - mBezierControl1.x).toDouble())
        }
        // 翻起页阴影顶点与touch点的距离
        ...
        rotateDegrees = Math.toDegrees(Math.atan2((mBezierControl2.y - mTouchY).toDouble(), (mBezierControl2.x - mTouchX).toDouble())).toFloat()
        ...
        if (hmg > mMaxLength)
            mCurrentPageShadow
                    .setBounds((mBezierControl2.x - 25).toInt() - hmg, leftx,
    ...
    private fun drawNextPageAreaAndShadow(canvas: Canvas, bitmap: Bitmap) {
        ...
        mBackShadowDrawable.setBounds(leftx, mBezierStart1.y.toInt(), rightx,
                (mMaxLength + mBezierStart1.y).toInt())//左上及右下角的xy坐标值,构成一个矩形
    ...
    private fun calcPoints() {
        ...
        mTouchToCornerDis = Math.hypot((mTouchX - mCornerX).toDouble(),
                (mTouchY - mCornerY).toDouble()).toFloat()

        mBezierEnd1 = getCross(PointF(mTouchX, mTouchY), mBezierControl1, mBezierStart1,
                mBezierStart2)
        mBezierEnd2 = getCross(PointF(mTouchX, mTouchY), mBezierControl2, mBezierStart1,
                mBezierStart2)
